            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    ...
                    data.forEach(product => {
                        ...
                        switch (product.estado_dinamico) {
                            case 'Disponible':
                                badgeClass = 'bg-success';
                                availabilityBadge = `<span class="badge ${badgeClass} position-absolute top-0 start-0 m-2" style="z-index: 1;">Disponible</span>`;
                                break;
                            case 'Por llegar':
                                badgeClass = 'bg-info text-dark';
                                availabilityBadge = `<span class="badge ${badgeClass} position-absolute top-0 start-0 m-2" style="z-index: 1;">Por llegar</span>`;
                                break;
                            case 'Agotado':
                                badgeClass = 'bg-danger';
                                availabilityBadge = `<span class="badge ${badgeClass} position-absolute top-0 start-0 m-2" style="z-index: 1;">Agotado</span>`;
                                break;
                        }
