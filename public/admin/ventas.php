...
// Total Vendido (respeta el filtro de fecha de la venta)
...
// Total Cobrado (respeta el filtro de fecha del pago y solo suma los confirmados)
$pagos_params = [];
$where_pagos_sql = " WHERE p.estado_confirmacion = 'Confirmado'";
if (!empty($start_date) && !empty($end_date)) {
    $where_pagos_sql .= " AND p.fecha_pago BETWEEN :start_date AND :end_date";
...
// --- CONSULTA PARA OBTENER LOS REGISTROS DE LA PÁGINA ACTUAL (CON PAGO CONFIRMADO) ---
$query = "
    SELECT 
        ...
        <div class="col-xl-4 col-md-6 mb-4">
            <div class="card border-left-success shadow h-100 py-2">
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            <div class="text-xs font-weight-bold text-success text-uppercase mb-1">Total Cobrado (Confirmado)</div>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    ...
                                    <?php if ($venta['total_pagado'] == 0 && ($venta['item_count'] ?? 0) == 0): ?>
                                    <form action="../../app/crud/ventas_process.php" method="POST" class="d-inline" onsubmit="return confirm('¿Estás seguro de que quieres ELIMINAR esta venta?');">
        ...
        <div class="card-footer">
            ...
            <nav>
                <ul class="pagination justify-content-center">
                    <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>"><a class="page-link" href="?<?php echo http_build_query(array_merge($filter_params_for_url, ['page' => $page - 1])); ?>">Anterior</a></li>
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <li class="page-item <?php echo ($page == $i) ? 'active' : ''; ?>"><a class="page-link" href="?<?php echo http_build_query(array_merge($filter_params_for_url, ['page' => $i])); ?>"><?php echo $i; ?></a></li>
                    <?php endfor; ?>
                    <li class="page-item <?php echo ($page >= $total_pages) ? 'disabled' : ''; ?>"><a class="page-link" href="?<?php echo http_build_query(array_merge($filter_params_for_url, ['page' => $page + 1])); ?>">Siguiente</a></li>
                </ul>
            </nav>
            <?php endif; ?>
        </div>
