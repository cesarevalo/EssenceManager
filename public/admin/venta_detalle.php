...
// --- Calcular totales BASADO SOLO EN PAGOS CONFIRMADOS ---
$total_pagado_en_usdt = 0;
$pagos_confirmados = array_filter($pagos_realizados, function($pago) {
    return $pago['estado_confirmacion'] == 'Confirmado';
});
foreach ($pagos_confirmados as $pago) {
...
$stmt_detalle = $db->prepare("SELECT vd.id, vd.cantidad, vd.precio_unitario_usdt, p.nombre as producto_nombre, p.costo_promedio_usdt FROM ventas_detalle vd JOIN productos p ON vd.id_producto = p.id WHERE vd.id_venta = ?");
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="ventas.php" class="btn btn-secondary mt-4"><i class="fas fa-arrow-left me-2"></i>Volver al Listado</a>
    </div>
    ...
    <div class="row">
        <div class="col-lg-4 mb-4">
            <div class="card mb-4">
                <div class="card-header"><h5>Resumen Financiero y de Venta</h5></div>
                <div class="card-body">
                    <p>Cliente: <strong><?php echo htmlspecialchars(strtoupper($venta['nombre'] . ' ' . $venta['apellido'])); ?></strong></p>
                    <p>Método de Entrega: <strong class="text-primary"><?php echo htmlspecialchars($venta['metodo_entrega']); ?>
                        <?php if($venta['metodo_entrega'] == 'Envío por Courier') echo "(Cobro en Destino)"; ?>
                    </strong></p>
                    <?php if($venta['metodo_entrega'] == 'Envío por Courier' && !empty($venta['direccion_envio'])): ?>
                        <p class="mb-0"><strong>Dirección de Envío:</strong></p>
                        <p class="text-muted"><?php echo nl2br(htmlspecialchars($venta['direccion_envio'])); ?></p>
                    <?php endif; ?>
                    <hr>
                    <p class="h5">Total Venta: <span class="float-end">$<?php echo number_format($venta['total_venta_usdt'], 2); ?></span></p>
                    <p class="h5 text-success">Total Pagado (Confirmado): <span class="float-end">$<?php echo number_format($total_pagado_en_usdt, 2); ?></span></p>
                    <hr>
            ...
            <div class="card">
                <div class="card-header"><h5>Registrar Pago (Confirmado por Admin)</h5></div>
                <div class="card-body">
                    <form action="../../app/crud/pagos_process.php" method="POST">
                        ...
                        <div class="mb-3"><label class="form-label">Tasa de Conversión a USDT</label><input type="number" step="0.00000001" class="form-control" name="tasa_conversion" value="1.00"><small class="form-text text-muted">Si es en VES, colocar la tasa. Si no, dejar en 1.</small></div>
                        ...
                        <button type="submit" class="btn btn-success w-100">Registrar Pago Confirmado</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form action="../../app/crud/ventas_process.php" method="POST">
                        <input type="hidden" name="action" value="add_product_to_venta"><input type="hidden" name="id_venta" value="<?php echo $id_venta; ?>">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="id_producto" name="id_producto" required>
                                ...
                                <?php foreach ($productos_disponibles as $prod): ?>
                                    <option value="<?php echo $prod['id']; ?>"><?php echo htmlspecialchars($prod['nombre']) . ' (Cod: ' . htmlspecialchars($prod['codigo_barra']) . ') - Stock: ' . $prod['stock'] . ' - $' . number_format($prod['precio_usdt'], 2); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="mb-3"><label for="cantidad" class="form-label">Cantidad a Vender</label><input type="number" class="form-control" name="cantidad" min="1" required></div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <table class="table table-sm">
                        <thead><tr><th>Producto</th><th>Cantidad</th><th>Costo Unit.</th><th>Precio Unit.</th><th>Ganancia</th><th>Acción</th></tr></thead>
                        <tbody>
                            <?php $ganancia_total_venta = 0; if (empty($detalles)): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="../../app/crud/ventas_process.php" method="POST" onsubmit="return confirm('¿Estás seguro? El stock será devuelto.');">
                                            <input type="hidden" name="action" value="remove_product_from_venta"><input type="hidden" name="id_venta_detalle" value="<?php echo $detalle['id']; ?>"><input type="hidden" name="id_venta" value="<?php echo $id_venta; ?>">
                        ...
                        <tfoot><tr class="table-light"><td colspan="4" class="text-end fw-bold">Ganancia Total de la Venta:</td><td class="fw-bold h5">$<?php echo number_format($ganancia_total_venta, 2); ?></td><td></td></tr></tfoot>
                    </table>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                     <table class="table table-sm">
                        <thead><tr><th>Fecha</th><th>Monto</th><th>Tasa</th><th>Monto USDT</th><th>Método</th><th>Ref.</th><th>Estado</th></tr></thead>
                        <tbody>
                            <?php if(empty($pagos_realizados)): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                     <?php 
                                        $estado_conf = htmlspecialchars($pago['estado_confirmacion']);
                                        $badge_conf_class = 'secondary';
                                        if ($estado_conf == 'Pendiente') $badge_conf_class = 'warning';
                                        if ($estado_conf == 'Confirmado') $badge_conf_class = 'success';
                                        if ($estado_conf == 'Rechazado') $badge_conf_class = 'danger';
                                        echo "<span class=\"badge bg-{$badge_conf_class}\">{$estado_conf}</span>";
                                    ?>
                                </td>
                            </tr>
                            <?php endforeach; ?>
